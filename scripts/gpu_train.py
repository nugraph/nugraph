...
def train(args):
    ...
    if args.name is None:
        if args.version is None:
            raise ValueError("You must specify at least one of --name or --version")
        args.name = args.version


    # Configure logger
    if args.logger == "wandb":
        logdir = pathlib.Path(os.environ["NUGRAPH_LOG"]) / str(args.name)
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is not available. This job requires 4 GPUs but torch.cuda.is_available() is False.")

    num_gpus = torch.cuda.device_count()
    print(f"[nugraph] Using {num_gpus} GPUs: CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES')}")
