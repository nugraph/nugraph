...
from .instance import InstanceNet
...
class NuGraph3(LightningModule):
    ...
    def __init__(self,
        ...
        self.instance_net = InstanceNet(planar_features,
                                        instance_features,
                                        planes)
        ...
        for p in planes:
            self.mix_net[p] = torch.nn.Sequential(
                torch.nn.Linear(in_features+planar_features+nexus_features+instance_features+interaction_features+1,
                                planar_features),
                torch.nn.Tanh(),
    ...
    def forward(self, x: TD, edge_index_plane: TD, edge_index_nexus: TD,
        ...
        for _ in range(self.num_iters):
            ...
            x_c, x_f = self.instance_net(m, batch)

            for p, net in self.mix_net.items():
                m[p] = torch.cat([x[p], x_p[p], x_n[p], x_c[p], x_f[p], x_e[batch[p]]], dim=-1)
                m[p] = net(m[p])
        ret = {}
        for decoder in self.decoders:
            ret.update(decoder(m, x_c, x_f, x_e, batch))
        return ret
